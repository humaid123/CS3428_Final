...
function getEmailsFromServer(isInbox, displayEmailsFunc) {
    ...
    $.ajax({
        type: "POST",
        url: SERVER_URL + '/secure/getEmails' + "?" + $.param({secret_token: token}), //this will be put in req.query
...
function createAccount(email, password, secret, isSpecialist, createAccountFunc) {
    $.ajax({
        type: "POST",
